...
- **The Old Town.** North of the cathedral a promenade opens onto the canal street and, beside it, three more avenues. Twenty-five lanes in four block columns cross the canal and avenues on bridges; each lane has ten townhouse plots a side, alternating open **courtyard** houses and roofed **shop** houses. Every house is one exhibition of two to six words: the 93 **root-family houses** come first (transcribed from Handy 990's root workshop; the floor plaque shows the root, its meaning in the current language, its Latin origin and the family's words, word banners highlight the root in gold and the plaque shows the formula such as `sub + port`), then 243 **theme houses** built from Handy 990's theme maps (the floor plaque shows the theme title in six languages and its group labels), 28 **word-family houses** (verb, noun and adjective forms of one stem), and 118 **level lanes** holding every remaining word above level 30, six to a house in alphabetical order by level band. A bordered plaque lies flat in each house's central floor, keeping its information readable without blocking the paintings on the back wall; a lintel sign names the house over its door, and lane signs at every bridge list the houses along that lane. Words that belong to several root families or to a landmark hang in each of their rooms and share one learning state. The **Bell Tower** closes the canal to the north and the **Observatory** stands on its hill beyond the walls.
...
- **Museum games / 博物館遊戲.** The game button opens six short activities. Most use the current Old Town house, with the port courtyard as the starting playroom elsewhere in the city. **Curator’s Quest / 小小策展人** gives a meaning clue and accepts the matching painting. **Restore the Labels / 找回單字標籤** temporarily replaces word banners with numbers; drag the offered label onto its painting or select the painting directly. **Listen and Step / 聽音踩單字** plays a word recording and lays two to four answer tiles on the floor; stand on a tile briefly or tap it. **Build a Word Family / 字根造字工坊** uses reviewed root-workshop splits: tap or drag word parts in order, learn spelling changes such as `sub → sup`, then find the matching painting. Houses without reviewed splits use the port courtyard. **Market Missions / 市集小任務** takes visitors to the actual market square to help three wooden characters: a baker, a furniture maker, and a potter. Click a figure or use its accessible shortcut, listen to a short request, and choose a contextual reply; incorrect replies receive specific feedback. Requests and character replies use browser English speech synthesis; vocabulary and example audio use the original recordings. **Memory Walk / 記憶散步** starts with a self-paced study of up to three paintings. It then hides room labels, captions, examples, and the floor word list while visitors recall a painting’s word, a spoken word’s meaning, and a missing word in an example. A missed word returns before the stamp can be earned. Every game offers hints and accessible answer buttons, practices up to three distinct words (preferring unchecked words, except for the three authored market missions), and ends with an audio review and one passport stamp per game per location. Stamps persist in local browser storage, separately from learned-word checkboxes. Games have no timer or penalties; ending them removes temporary tiles and characters and restores all labels and floor information.
...
Game checks cover all six activities, drag-and-tap word assembly, reviewed spelling changes, walking and clicking floor tiles, clicking market characters, spoken request dispatch and contextual retries, memory recall and missed-word retries, phone layouts in Traditional Chinese, backward-compatible passport storage, and complete scene cleanup. Run `npx playwright test tests/games.spec.ts tests/more-games.spec.ts` for this focused suite.
